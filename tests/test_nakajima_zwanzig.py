...
def _build_ground_truth_Us(
    model: DiscreteTimeGroundTruthNZGME, L: torch.Tensor, n_steps: int
) -> torch.Tensor:
...
def test_get_Ks_from_Us_left_and_right_recursions_agree_for_noncommuting_case(
    ...
    Us = _build_example_Us(
        simple_column_stochastic_matrix, simple_inhomogeneous_column_stochastic_stack
    )
    Ks = get_Ks_from_Us(Us, verbose=False)
    ...
    assert not torch.allclose(
        stack[1] @ stack[2], stack[2] @ stack[1], atol=1e-12, rtol=0.0
    )
    assert not torch.allclose(Us[1] @ Ks[1], Ks[1] @ Us[1], atol=1e-12, rtol=0.0)
...
def test_transfer_matrix_nzgme_integrate_matches_transfer_matrix_evolution(
    ...
    returned_Ks, dists = model.integrate(
        Us.shape[0] - 1, kernels=Ks, initial_dist=initial_dist, verbose=False
    )
...
def test_transfer_matrix_nzgme_integrate_computes_kernels_when_omitted(
    ...
    returned_Ks, dists = model.integrate(
        Us.shape[0] - 1, initial_dist=initial_dist, verbose=False
    )
    expected_dists = torch.stack([Us[t] @ initial_dist for t in range(Us.shape[0])])
...
def test_discrete_time_ground_truth_nzgme_get_U_dtgme_matches_manual_computation(
    ...
    for m in [0, 1, 3]:
        ...
        expected = (
            simple_macrostate_indicator.T
            @ matrix_power(model.L, m)
            @ p_micro_given_macro
        )
...
def test_discrete_time_ground_truth_nzgme_kernels_match_transfer_matrix_nzgme(
    simple_column_stochastic_matrix: torch.Tensor, simple_groups: list[list[int]]
) -> None:
    ...
    assert torch.allclose(transfer_kernels, ground_truth_kernels, atol=1e-12, rtol=0.0)
...
def test_discrete_time_ground_truth_nzgme_integrate_matches_transfer_matrix_nzgme(
    ...
    ground_truth_kernels, ground_truth_dists = model.integrate(
        deltat=1.0, n_steps=n_steps, initial_dist=initial_dist, verbose=False
    )
    returned_transfer_kernels, transfer_dists = transfer_model.integrate(
        n_steps, kernels=transfer_kernels, initial_dist=initial_dist, verbose=False
    )

    assert torch.allclose(
        ground_truth_kernels, returned_transfer_kernels, atol=1e-12, rtol=0.0
    )
...
def test_discrete_time_ground_truth_nzgme_markov_jump_process_matches_transfer_matrix_nzgme(
    simple_cycle_rates: torch.Tensor, simple_groups: list[list[int]]
) -> None:
    ...
    transfer_model = TransferMatrixNZGME(Us)
    transfer_kernels = transfer_model.get_Ks(verbose=False)
    direct_kernels = torch.stack([model.get_K_dtgme(L, m) for m in range(n_steps)])

    ground_truth_kernels, ground_truth_dists = model.integrate(
        deltat=deltat, n_steps=n_steps, initial_dist=initial_dist, verbose=False
    )
    returned_transfer_kernels, transfer_dists = transfer_model.integrate(
        n_steps, kernels=transfer_kernels, initial_dist=initial_dist, verbose=False
    )
...
def test_discrete_time_ground_truth_nzgme_rejects_invalid_inputs(
    ...
    inhomogeneous_process = MarkovChain(
        simple_inhomogeneous_column_stochastic_stack, dt=0.5, device="cpu"
    )
...
def test_transfer_matrix_nzgme_rejects_invalid_shapes(
    ...
    with pytest.raises(AssertionError, match="kernels must be 3D"):
        model.integrate(
            Us.shape[0] - 1,
            kernels=torch.zeros((3, 4), dtype=torch.float64),
            verbose=False,
        )
    with pytest.raises(AssertionError, match="kernels must have square slices"):
        model.integrate(
            Us.shape[0] - 1,
            kernels=torch.zeros((3, 4, 3), dtype=torch.float64),
            verbose=False,
        )
