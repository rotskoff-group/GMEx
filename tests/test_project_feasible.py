...
def test_sinkhorn_projects_sampled_flux_to_target_marginals(
    ...
    assert (
        torch.max(torch.abs(flux.sum(dim=1) - simple_stationary_dist)).item() <= 1e-10
    )
    assert (
        torch.max(torch.abs(flux.sum(dim=0) - simple_stationary_dist)).item() <= 1e-10
    )
...
def test_reversible_commuting_matches_kru_for_identity_uprev() -> None:
    ...
    kru = KnightRuizUcarScaler(
        min_entry=1e-24, max_iters=100000, tol=1e-12, progress=False
    )
...
def test_reversible_commuting_projects_to_feasible_subset(
    ...
    assert torch.max(torch.abs(flux - flux.T)).item() <= 1e-12
    assert (
        torch.max(torch.abs(flux.sum(dim=1) - reversible_stationary_dist)).item()
        <= 1e-10
    )
    assert (
        torch.max(
...
def test_project_us_matches_manual_lagwise_projection_nonreversible(
    ...
    transitions = torch.stack(
        [
            column_normalize(count_matrices[i], fallback_col=simple_stationary_dist)
            for i in range(count_matrices.shape[0])
        ]
    )
