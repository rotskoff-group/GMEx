...
from gmex.markov_process import GillespieMJP, MarkovChain, MarkovJumpProcess, MarkovProcess
...
def test_get_initial_states_samples_stationary_distribution():
    ...
    assert torch.equal(initial_states, torch.ones(5, dtype=torch.long, device=sim.device))
...
def test_get_initial_states_uses_explicit_initial_state():
    ...
    sim = DummyMarkovProcess(torch.tensor([0.25, 0.25, 0.25, 0.25], dtype=torch.float64))

    initial_states = sim._get_initial_states(n_trajs=5, initial_state=2)

    assert torch.equal(initial_states, torch.full((5,), 2, dtype=torch.long, device=sim.device))
...
def test_markov_jump(device_for_testing, simple_cycle_rates):
    ...
    expected_barriers = linalg.toeplitz([0.0, -np.log(2)/2, float('inf'), -np.log(2)/2])
    ...
    expected_forces = linalg.toeplitz([0.0, -np.log(2), 0.0, np.log(2)],
                                      [0.0, np.log(2), 0.0, -np.log(2)])
    ...
    initial_dist = torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=torch.float64, device=sim.device)
    ...
    assert observed_dists.shape == (len(times), sim.n_states), "Generated distributions have wrong shape"
    assert torch.allclose(observed_dists[0], initial_dist), "Distribution at t=0 should equal initial distribution"
    ...
    assert torch.all(observed_dists >= -1e-12), "Generated distributions must remain nonnegative"
    assert torch.allclose(
        observed_dists,
        expected_dists,
        rtol=1e-8,
...
def test_homogeneous_markov_chain(
    ...
    """
    sim = MarkovChain(simple_column_stochastic_matrix, dt=0.5, device=device_for_testing)
    ...
    assert sim.Gs is None, "Homogeneous Markov chain should not store time-dependent propagators"
    ...
    assert torch.allclose(observed_statdist, expected_statdist), "Stationary distribution incorrect"
    ...
    expected_epr2 = torch.nan_to_num(
    ...
    assert torch.allclose(observed_dists, expected_dists), "Propagated distributions incorrect"
    ...
    assert torch.equal(observed_states[:, 0], torch.full((2,), 1, dtype=torch.long, device=sim.device)), (
        "Initial sampled states incorrect"
    )
    assert torch.all((observed_states >= 0) & (observed_states < sim.n_states)), "Sampled states out of range"

    # test 7: verify deterministic first-passage times
    observed_fpts = sim.first_passage_times(1, 1, n_trajs=3)
    assert torch.equal(observed_fpts, torch.zeros(3, dtype=torch.long, device=sim.device)), (
    ...
    expected_dwell = torch.zeros((sim.n_states, 4), dtype=torch.float64, device=sim.device)
    expected_dwell[:, 1:] = (
        stay_probs.unsqueeze(1) ** torch.arange(3, device=sim.device, dtype=torch.float64)
    ) * (1.0 - stay_probs).unsqueeze(1)
    assert torch.allclose(observed_dwell, expected_dwell), "Dwell-time probabilities incorrect"
...
def test_inhomogeneous_markov_chain(
    ...
    """
    sim = MarkovChain(simple_inhomogeneous_column_stochastic_stack, dt=0.5, device=device_for_testing)

    # test 1: verify inhomogeneous Markov-chain parameterization
    expected_props = simple_inhomogeneous_column_stochastic_stack.to(sim.device).to(torch.float64)
    assert sim.n_states == 4, "Number of states incorrect"
    assert sim.dt == 0.5, "Timestep size incorrect"
    assert sim.L is None, "Inhomogeneous Markov chain should not store a single propagator"
    ...
    assert torch.allclose(sim._get_step_propagator(0), expected_props[1]), "Step-0 propagator incorrect"
    assert torch.allclose(sim._get_step_propagator(1), expected_props[2]), "Step-1 propagator incorrect"
    assert torch.allclose(sim._get_final_propagator(), expected_props[2]), "Final propagator incorrect"
    ...
    assert observed_flux.shape == (2, sim.n_states, sim.n_states), "Stationary flux has wrong shape"
    assert torch.allclose(observed_flux, expected_flux), "Stationary flux incorrect"
    ...
    assert torch.allclose(observed_epr2, expected_epr2), "Second-order local entropy production rates incorrect"
    ...
    expected_dists = torch.stack([
        ...
        expected_props[2] @ (expected_props[2] @ (expected_props[1] @ initial_dist)),
    ])
    assert torch.allclose(observed_dists, expected_dists), "Propagated distributions incorrect"
...
def test_gillespie(device_for_testing, simple_cycle_rates):
    ...
    expected_transition_probs = np.array(
        [theoretical_prob_to_1, theoretical_prob_to_3]) * total_transitions
    
    _, p_transitions = stats.chisquare(observed_transition_probs,
    ...
    assert relative_error < 0.1, f"Mean waiting time error too large: {
       relative_error}"
