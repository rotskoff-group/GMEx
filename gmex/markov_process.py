...
from .utils import get_random_init_dist, check_column_stochastic_matrix, column_stochastic_lim_dist
...
class MarkovProcess(ABC):
    ...
    def __init__(self, device: str | torch.device | None = None):
        self.device = device if device else ( # set device to GPU if available
    ...
    @torch.no_grad()
    def _get_initial_states(self, n_trajs: int, initial_state: int | None = None) -> torch.Tensor:
        ...
        if initial_state is None:
            initial_states = torch.multinomial(
                self.stationary_distribution(), n_trajs, replacement=True).to(self.device)
        else:
            initial_states = torch.full((n_trajs,), initial_state, dtype=torch.long, device=self.device)
...
class MarkovChain(MarkovProcess):
    ...
    """
    @torch.no_grad()
    def __init__(self, prop: torch.Tensor, dt: float, device: str | torch.device | None = None) -> None:
        super().__init__(device)
        if prop.ndim == 2:
            if prop.shape[0] != prop.shape[1]:
                raise ValueError('prop must be square.')
            check_column_stochastic_matrix(prop, name='prop')
            self.L = (prop / prop.sum(dim=0, keepdim=True)).to(self.device).to(torch.float64)
            self.Gs = None
        elif prop.ndim == 3:
            if prop.shape[0] < 1:
            ...
            self.L = None
            self.Gs = (prop / prop.sum(dim=1, keepdim=True)).to(self.device).to(torch.float64)
            ...
            if not torch.allclose(self.Gs[0], eye, rtol=1e-8, atol=1e-8):
                raise ValueError('prop[0] must be (approximately) identity when prop is 3D.')
        else:
            raise ValueError('prop must be an (M, M) matrix or an (N, M, M) stack of matrices.')
    ...
    def _get_initial_states(self, n_trajs: int, initial_state: int | None = None) -> torch.Tensor:
        ...
        if initial_state is None and self.Gs is not None:
            warn('Sampling from stationary distribution of final TPM for inhomogeneous Markov chain.',
                 UserWarning)
    ...
    @torch.no_grad()
    def _to_states(self, states: int | list[int] | torch.Tensor, name: str) -> torch.Tensor:
        ...
        if isinstance(states, int):
            ...
            states = torch.as_tensor(states, dtype=torch.long, device=self.device).flatten()
    ...
    @torch.no_grad()
    def propagate(self, times: torch.Tensor, initial_dist: torch.Tensor | None = None) -> torch.Tensor:
        ...
        """
        assert self._parameterized, 'Markov chain must be parameterized before propagation'
        if times.ndim != 1:
        ...
        if not torch.allclose(steps.to(torch.float64) * self.dt, times, rtol=1e-8, atol=1e-8):
        ...
        dists = torch.empty((times.shape[0], self.n_states), dtype=torch.float64, device=self.device)
        dist = initial_dist
        current_step = 0
    ...
    @torch.no_grad()
    def sample(self, n_steps: int, n_trajs: int = 1, initial_state: int | None = None, verbose: bool = False):
        """
        ...
        """
        assert self._parameterized, 'Markov chain must be parameterized before simulation'
                
        times = torch.tile(torch.arange(
            n_steps + 1, dtype=torch.float64, device=self.device) * self.dt, (n_trajs, 1))
        ...
        for step in tqdm(range(n_steps), desc='Timesteps', disable=not verbose):
            ...
            states[:, step + 1] = torch.searchsorted(
                row_cdf.contiguous(),
                random_choices[:, step].contiguous().unsqueeze(1)
            ).squeeze(1)
    ...
    def first_passage_times(self,
        ...
        if start_dist is None:
            ...
            start_dist = torch.as_tensor(start_dist, dtype=torch.float64, device=self.device).flatten()
        ...
        while alive.any():
            ...
            random_choices = torch.rand((idx.numel(), 1), dtype=row_cdf.dtype,
                                        device=self.device, generator=generator)
            next_states = torch.searchsorted(row_cdf.contiguous(), random_choices, right=False).squeeze(-1)
    ...
    def dwell_probabilities(self, max_timestep: int = 1000) -> torch.Tensor:
        ...
        prob_dwell = torch.zeros((self.n_states, max_timestep), dtype=dtype, device=self.device)
        if max_timestep == 1:
            return prob_dwell
        ...
        prob_alive = torch.ones((self.n_states, max_timestep), dtype=dtype, device=self.device)
    ...
    def get_2nd_local_epr(self) -> torch.Tensor:
        ...
        flux = self.get_flux()
        if torch.any(
            ((flux == 0) & (flux.transpose(-1, -2) != 0)) | ((flux != 0) & (flux.transpose(-1, -2) == 0))
        ):
            raise ValueError('Propagators must be microscopically reversible for physical EPR.')
    ...
    def get_3rd_local_epr(self) -> torch.Tensor:
        ...
        if self.Gs is None or len(self.Gs) < 3:
            return torch.tensor(0.0) # 3rd-order EPR vanishes for homogeneous Markov chain
...
class MarkovJumpProcess(MarkovProcess):
    ...
    def parameterize_from_rates(self, rates: torch.Tensor):
        ...
        """
        assert rates.ndim == 2 and rates.shape[0] == rates.shape[1], 'rates must be square'
        ...
        self.wells = torch.zeros(self.n_states, device=self.device) # arbitrarily set wells to 0
        ...
        self.forces = torch.nan_to_num(self.forces, nan=0.0, posinf=float('inf'), neginf=-float('inf'))
        
        self._set_parameterized() # set parameterization state to True

    @torch.no_grad()
    def parameterize_from_energies(self, wells: torch.Tensor, barriers: torch.Tensor, forces: torch.Tensor):
        ...
        forces : torch.Tensor
            Element i, j is energy due to force driving from vertex i to j.
        """
        assert wells.shape[0] == barriers.shape[0] == forces.shape[0], 'parameters must have same shape along first axis'
        ...
        self.rates = torch.exp(torch.tile(self.wells, (self.n_states, 1)) - self.barriers + self.forces / 2.0)
    ...
    def stationary_epr(self) -> float:
        ...
        epr_summands_doubled = (influxes - influxes.T) * torch.log(influxes / influxes.T)
        return epr_summands_doubled.nansum().item() / 2.0

    @torch.no_grad()
    def generate(self, times: torch.Tensor, initial_dist: torch.Tensor | None = None) -> torch.Tensor:
        """Explicitly propagate fine-grained distribution using transition-rate matrix.
...
class GillespieMJP(MarkovJumpProcess):
    ...
    def sample(self,
        ...
        times = torch.zeros(n_trajs, n_steps + 1, dtype=torch.float64, device=self.device)
        current_times = torch.zeros(n_trajs, dtype=torch.float64, device=self.device)

        # Generate all random numbers at once
        random_times = torch.rand(n_trajs, n_steps, dtype=torch.float64, device=self.device)
        random_choices = torch.rand(n_trajs, n_steps, device=self.device)
        ...
        for step in tqdm(range(n_steps), desc='Timesteps', disable=not verbose):
            ...
            if valid_mask.any():
                ...
                dt = -torch.log(random_times[valid_mask, step]) / total_rates[valid_mask]
                current_times[valid_mask] += dt

                # Find next states
                cumsum_rates = torch.cumsum(current_rates[valid_mask], dim=1) / total_rates[valid_mask].unsqueeze(1)
