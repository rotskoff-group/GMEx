...
class DeeptimeReversibleU:
    ...
    @torch.no_grad()
    def fit(self, max_iters: int = 1000000, sparse: bool = False) -> Tuple[torch.Tensor, Dict]:
        ...
        try:
            ...
            raise RuntimeError('Deeptime failed. Try increasing max_iters or setting sparse to False.')
...
class MirrorDescentStochasticMatrix(ABC):
    ...
    """
    def __init__(self, C: torch.Tensor, P: torch.Tensor,
                 tol: float = 1e-12,
    ...
    def _accept_proposal(self,
                         ...
                         objectives_try: Dict) -> Tuple[bool, bool] | Tuple[Dict, Dict]:
        """Decide whether to accept proposal."""
        raise NotImplementedError

    @abstractmethod
    def _accept_convergence(self,
                            estimates_before: Dict,
                            estimates: Dict,
                            objectives_before: Dict,
                            objectives: Dict) -> Tuple[bool, float, float]:
    ...
    def fit(self,
            ...
            verbose: bool = True,
            log_every: int = 1,
            postfix_callback: Callable[[Dict], None] | None = None) -> Tuple[Dict, Dict, Dict]:
        ...
        for it in iter_range:
            ...
            for _ in range(max_iters_ls):
                ...
                if estimates_accept: # if line search is accepted or if line_search is False
                    estimates = estimates_accept
                    objectives = objectives_accept
                    accepted = True
                    break # and break line-search loop
                eta_try = eta_try * eta_shrink # if LS iter is not accepted, eta decreases for LS next iter
        ...
        if not info['converged']:
            if info['ls_failed']:
                warnings.warn(f'Optimization terminated after outer iteration exceeded {max_iters_ls} line searches: eta_try = {eta_try:.2e}.')
            else:
                warnings.warn(f'Convergence failed: objective increase or iterate change in last iteration greater than tolerance {self.tol}.')
...
class MirrorDescentU(MirrorDescentStochasticMatrix):
    ...
    def _get_objectives(self, estimates: Dict) -> Dict:
        """Evaluate objective-function values."""
        return {'obj': float(get_log_likelihood(
    ...
    def _propose_estimates(self,
                           ...
                           eta_try: float,
                           grad_clip: float,
                           projector: SinkhornKnoppScaler | KnightRuizUcarScaler) -> Dict:
    ...
    def _accept_convergence(self,
                            ...
                            objectives: Dict) -> Tuple[bool, float, float]:
        """Decide whether optimization has converged."""
        delta_est = float(torch.abs(
            estimates['F'] - estimates_before['F']
        ).max().detach().cpu().item())
        delta_obj = objectives['obj'] - objectives_before['obj']
        return delta_est <= self.tol and abs(delta_obj) <= self.tol * max(
    ...
    def fit(self,
            ...
            verbose: bool = True,
            log_every: int = 1,
            postfix_callback: Callable[[Dict], None] | None = None) -> Tuple[torch.Tensor, Dict]:
        ...
        info['col_sum_err'] = (
            U.sum(dim=0) - 1.0
        ).abs().sum().detach().cpu().item() # final deviation from stochasticity
        info['lim_dist_err'] = tv_distance(
            U @ self.P, self.P
        ) # final error in limiting distribution
        info['llh_final'] = float(get_log_likelihood(
            self.C, U, min_entry=self.min_entry
        ).detach().cpu().item()) # final log-likelihood
        info['obj_final'] = float(objectives['obj']) # final objective-function value
        info['row_proj_err'] = float(proj_info['row_err']) # final error in flux row marginals
        info['kld_final'] = float(kl_divergence(
            column_normalize(self.C, fallback_col=self.P), U, eps=self.min_entry, reduce_dim=0
        ).sum().detach().cpu().item()) # final KL divergence from naive transfer matrix
        info['lim_sigma'] = float(torch.nan_to_num(
            F * torch.log(F / F.T), nan=0.0, posinf=0.0, neginf=0.0
        ).sum().detach().cpu().item()) # final estimated stationary entropy production, also ligma
        if not self.reversible:
            info['col_proj_err'] = float(proj_info['col_err']) # final error in flux column marginals
...
class MirrorDescentG(MirrorDescentStochasticMatrix):
    ...
    def __init__(self,
        ...
        if C.shape != U_prev.shape:
            raise ValueError(f'C and U_prev must have the same shape, got {tuple(C.shape)} and {tuple(U_prev.shape)}.')
    ...
    def _initialize_estimates(self,
        ...
        """Initialize estimates."""
        try:
            check_stationary_column_stochastic_matrix(self.U_prev, self.P, db=self.reversible, name='U_prev')
        except ValueError as err:
            warnings.warn(f"Attempting matrix-scaling fix, but U_prev failed check: {err}", stacklevel=2)
        ...
        self._U_prev_full_rank = torch.linalg.matrix_rank(self.U_prev, atol=self.tol) == self.n_states
    ...
    def _propose_estimates(self,
                           ...
                           eta_try: float,
                           grad_clip: float,
                           projector: SinkhornKnoppScaler | ReversibleCommutingIProjector) -> Dict:
        """Propose updated estimates, generally outside of feasible set."""
        grad_max_amp = grad.abs().max()
        if grad_max_amp > grad_clip:
            grad = grad * grad_clip / grad_max_amp
        ...
        return {'G': G_try, 'GD': GD_try, 'Cw': estimates['Cw'], 'proj_info': proj_info_try}
    ...
    def _accept_convergence(self,
        ...
        """Decide whether optimization has converged."""
        delta_est = float(torch.abs(
            estimates['GD'] - estimates_before['GD']
        ).max().detach().cpu().item())
        delta_obj = objectives['obj'] - objectives_before['obj']
        ...
        return delta_obj <= self.tol * max( # optimizer is nonunique
            1.0, abs(objectives_before['obj'])
        ), delta_est, delta_obj
    ...
    def fit(self,
        ...
        if G0 is not None:
            if type(G0) != torch.Tensor:
                raise ValueError(f'Expected G0 to be type torch.Tensor, got type {type(G0)}')
            if G0.ndim != 2 or G0.shape[0] != G0.shape[1] or G0.shape[0] != self.n_states:
                raise ValueError(
        ...
        if self.reversible:
            projector = ReversibleCommutingIProjector(
                self.min_entry, max_iters_proj, self.tol,
        ...
        info['row_proj_err'] = float(proj_info['row_err']) # final error in flux row marginals
        info['kld_final'] = float(kl_divergence(
            self._G0, G, eps=self.min_entry, reduce_dim=0
        ).sum().detach().cpu().item()) # final KL divergence from naive transition matrix
        info['lim_sigma_U'] = float(torch.nan_to_num(
            UD * torch.log(UD / UD.T), nan=0.0, posinf=0.0, neginf=0.0
        ).sum().detach().cpu().item()) # final estimated stationary EPR of U
        info['lim_sigma_G'] = float(torch.nan_to_num(
            GD * torch.log(GD / GD.T), nan=0.0, posinf=0.0, neginf=0.0
        ).sum().detach().cpu().item()) # final estimated stationary EPR of G
        if self.reversible:
            info['commutator_norm'] = float(torch.linalg.matrix_norm(
...
def get_reversible_Us_deeptime(Cs: torch.Tensor,
    ...
    lag_iter = tqdm(range(1, Cs.shape[0]), desc='Lags', leave=False, disable=not verbose)
    for lag in lag_iter:
...
def get_Us_mle(Cs: torch.Tensor,
               lim_dist: torch.Tensor,
               tol: float = 1e-12,
               min_entry: float = 1e-24,
               eta: float = 1.0,
...
def get_Gs_mle(Cs: torch.Tensor,
               lim_dist: torch.Tensor,
               tol: float = 1e-12,
               min_entry: float = 1e-24,
               eta: float = 1.0,
    ...
    if precomputed is not None:
        ...
        if Gs_precomputed.shape[1:] != Cs.shape[1:]:
            raise ValueError('Precomputed propagators must have same 1st and 2nd dimension as Cs.')
        ...
        if reversible and Gs_precomputed.shape[0] > 2:
            if not torch.allclose(Gs_precomputed[1], Gs_precomputed[2], atol=tol):
                raise ValueError('Gs_precomputed[1] must equal Gs_precomputed[2] in reversible case.')
    ...
    for lag in lag_iter:
        if lag < start_lag:
            ...
            metrics[lag] = info_precomputed[lag] if lag in info_precomputed else info_precomputed[str(lag)]
        else:
            if reversible and lag == 2:
                ...
                estimator = MirrorDescentG(
                    Cs[lag], Us[lag - 1], lim_dist, tol=tol, min_entry=min_entry, reversible=reversible
                )
                G, info = estimator.fit(
                    ...
                    verbose=False,
                    G0=None if lag == 1 else Gs[lag - 1], # initialize with previous G at lags other than first
                    postfix_callback=_update_outer_postfix
                )
            Gs[lag] = G
            Us[lag] = G @ Us[lag - 1]
