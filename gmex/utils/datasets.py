...
class StateSeqDataset(Dataset):
    def __init__(self,
                 ...
                 data: Tensor | List | None = None) -> None:
        ...
        if data is not None:
            ...
            if not isinstance(times, Tensor):
                raise ValueError(f'times must be a torch.Tensor, got type {type(times)}')
            if not isinstance(states, Tensor):
                raise ValueError(f'states must be a torch.Tensor, got type {type(states)}')
            if data is not None:
                raise ValueError('If times and states are passed, data must be None.')
    ...
    def _get_metadata(self) -> None:
        ...
        if isinstance(self.data, list):
            if not isinstance(self.data[0], Tensor):
                raise ValueError(f'Records must be tensors, got type {type(self.data[0])}.')
            ...
            raise TypeError(f'Unrecognized data type: {type(self.data)}')

        # get total observations
        records = self.data if isinstance(self.data, list) else [self.data]
        concatenated_records = torch.cat(records).to(int).cpu().numpy()
        ...
        self.frac_transitionless = float(self.n_transitionless_contexts / self.n_contexts)

        # get fraction of transitions lost to resampling
        if self.resampled == True and self.n_transitions_raw != 0:
            self.frac_transitions_lost = 1.0 - self.n_transitions_resampled / self.n_transitions_raw
        else:
    ...
    def get_count_matrices(self,
        ...
        for i in tqdm(range(max_save), desc='Lags', disable=not verbose, leave=False):
            ...
            counts_acc = torch.zeros(self.n_states ** 2, dtype=torch.long) # accumulated counts
...
class MultiSeqDataset(StateSeqDataset):
    def __init__(self,
        ...
        """
        super().__init__(timestep, context_length, times=times, states=states, data=data)

        if data is not None:
            self.data = [torch.tensor(rec, dtype=int) for rec in data if len(rec) > self.context_length]
        elif times is not None and states is not None:
            self.data = []
        ...
        if len(self.data) < 1:
            raise ValueError(f'No records longer than context_length {self.context_length}.')    

        # get metadata
        self.n_iters_cumsum = torch.Tensor([
            len(s) - self.context_length for s in self.data
            ]).to(torch.long).cumsum(dim=0)
        self._get_metadata()
        if verbose:
            print(self.metadata)
        
    def __len__(self):
        return self.n_iters_cumsum[-1]
    ...
    def __getitem__(self, idx):
        ...
        i = torch.searchsorted(self.n_iters_cumsum, idx + 1)
        # get start index of context slice
        j = idx - (self.n_iters_cumsum[i - 1] if i > 0 else 0)
...
class SingleSeqDataset(StateSeqDataset):
    def __init__(self,
        ...
        verbose : bool, optional
            Whether to print metadata.
        """
        super().__init__(timestep, context_length, data=data, times=times, states=states)

        if data is not None:
            self.data = torch.tensor(data, dtype=int)
        elif times is not None and states is not None:
            _, self.data, n_transitions = fixed_timestep_resample(times, states, self.dt)
            self.n_transitions_raw = n_transitions[0]
            self.n_transitions_resampled = n_transitions[1]
        if len(self.data) <= self.context_length:
            raise ValueError(f'Record of length {len(self.data)} shorter than context length {self.context_length}.')
...
@torch.no_grad()
def get_split(timestep: float,
              context_length: int,
              times: torch.Tensor | None = None,
              states: torch.Tensor | None = None,
              data: List[int] | List[List[int]] | None = None,
              train_frac: float = 0.8,
              verbose: bool = False) -> StateSeqDataset | tuple[StateSeqDataset, StateSeqDataset]:
    ...
    """
    ds1 = lambda dt, cl, t, s, d, v:\
        SingleSeqDataset(dt, cl, times=t, states=s, data=d, verbose=v)
    ds2 = lambda dt, cl, t, s, d, v:\
        MultiSeqDataset(dt, cl, times=t, states=s, data=d, verbose=v)
    if data is not None:
        if type(data[0]) == int:
            ds = ds1
            ...
            raise ValueError(f'data must be a list of int or list, got a list of {type(data[0])}')
    elif times is not None and states is not None:
        if states.ndim == 1:
            ds = ds1
        elif states.ndim == 2:
    ...
    if train_frac == 1:
        ...
        if data is not None:
            ...
            if verbose:
                print('Training data:')
            train_data = ds(timestep, context_length, None, None, data[:ntrain], verbose)
            if verbose:
                print('Validation data:')
            valid_data = ds(timestep, train_data.context_length, None, None, data[ntrain:], verbose)
        else:
            ntrain = int(train_frac * states.shape[0])
            if verbose:
                print('Training data:')
            train_data = ds(timestep, context_length, times[:ntrain], states[:ntrain], None, verbose)
            if verbose:
                print('Validation data:')
            valid_data = ds(timestep, train_data.context_length, times[ntrain:], states[ntrain:], None, verbose)
...
@torch.no_grad()
def fixed_timestep_resample(times: torch.Tensor,
                            states: torch.Tensor,
                            timestep: float,
                            return_transition_cts: bool = True) -> tuple:
    ...
    interpolated_trajectory = interp1d(times_np, states_np, kind='previous', bounds_error=False, fill_value="extrapolate")
    ...
    new_times = float(timestep) * torch.arange(last_step + 1, dtype=torch.float64, device=times.device)
    new_states = torch.tensor(interpolated_trajectory(new_times.cpu().numpy()), dtype=torch.int64)
...
def load_times_macrostates(ds: str | Path, device: str | torch.device) -> tuple:
    ...
    if STATE_LIST_FN in datafns:
        ...
        if isinstance(data[0], list):
            observed_macrostates = set(x for sublist in data for x in sublist)
        elif isinstance(data[0], int):
            observed_macrostates = set(data)
        else:
            raise ValueError(f'data must be a list of int or list, got list of {type(data[0])}')
        expected_macrostates = set([i for i in range(max(observed_macrostates) + 1)])
    elif STATE_TENSOR_FN in datafns:
        microstates = torch.load(datadir / STATE_TENSOR_FN, weights_only=True).to(device)
        if GROUPS_FN in datafns:
        ...
        raise ValueError(f'Dataset {datadir} must contain {STATE_LIST_FN} xor {STATE_TENSOR_FN} and {TIMES_FN}.')
