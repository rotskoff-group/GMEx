...
from .markov_process import MarkovProcess, MarkovChain, MarkovJumpProcess
...
class NakajimaZwanzigGME:
    ...
    def integrate(self,
        ...
        dists = allocate_dists(n_steps, n_macrostates, initial_dist=initial_dist).to(device)

        for i in tqdm(range(n_steps), desc='Timesteps integrated', disable=not verbose, leave=False):
...
class GroundTruthNZGME(NakajimaZwanzigGME):
    ...
    def __init__(self, process: MarkovProcess, groups: list):
        ...
        self.macrostate_indicator = get_macrostate_indicator(self.groups).to(self.device)
        
        # compute the macrostate stationary distribution
        self.stationary_distribution_micro = self.process.stationary_distribution()
        self.stationary_distribution_macro = self.macrostate_indicator.T @ self.stationary_distribution_micro
        ...
        self.mz_projector_Q = torch.eye(self.process.n_states).to(self.device) - self.mz_projector_P
...
class DiscreteTimeGroundTruthNZGME(GroundTruthNZGME):
    ...
    def __init__(self, process: MarkovProcess, groups: list):
        ...
        if isinstance(self.process, MarkovChain):
            ...
            raise ValueError('process must be MarkovChain or MarkovJumpProcess')
    ...
    def get_U_dtgme(self, L: torch.Tensor, m: int) -> torch.Tensor:
        ...
        """
        return self.macrostate_indicator.T @ matrix_power(L, m) @ self.p_micro_given_macro
    ...
    def get_K_dtgme(self, L: torch.Tensor, m: int) -> torch.Tensor:
        ...
        """
        return self.macrostate_indicator.T @ L @ matrix_power(
            self.mz_projector_Q.T @ L, m) @ self.p_micro_given_macro

    @torch.no_grad()
    def integrate(self, deltat: float, n_steps: int,
        ...
        """
        if isinstance(self.process, MarkovChain):
            if not torch.isclose(torch.tensor(deltat % self.process.dt), torch.tensor(0.0)):
        ...
        if kernels is not None:
            ...
            assert kernels.shape[1] == kernels.shape[2], 'kernels must have square slices'
            kernels = kernels.to(torch.float64)
        else:
            kernels = torch.zeros(n_steps, self.n_macrostates, self.n_macrostates, dtype=torch.float64).to(self.device)
            kernel_prefactor = self.macrostate_indicator.T @ L
            irrelevant_L = self.mz_projector_Q.T @ L
            for m in tqdm(range(n_steps), desc='Kernels computed', disable=not verbose, leave=False):
                kernels[m] = kernel_prefactor @ self.p_micro_given_macro
                kernel_prefactor = kernel_prefactor @ irrelevant_L
        
        dists = super().integrate(kernels, n_steps, self.n_macrostates, self.device, initial_dist=initial_dist, verbose=verbose)
...
class TransferMatrixNZGME(NakajimaZwanzigGME):
    ...
    def integrate(self,
        ...
        dists = super().integrate(
            kernels, n_steps, self.n_macrostates, self.device, initial_dist=initial_dist, verbose=verbose
        )
        return kernels, dists
...
@torch.no_grad()
def convolve_sum(max_delay: int, kernels: torch.Tensor, dists: torch.Tensor) -> torch.Tensor:
    ...
    """
    assert kernels.shape[1] == kernels.shape[2], 'kernel at each timestep must be square'
    assert kernels.shape[1] == dists.shape[1], 'dist support cardinality must equal second dimension of kernels'

    if kernels.shape[0] <= max_delay: # case with max delay more than available kernels
        return (kernels @ dists[max_delay + 1 - kernels.shape[0]:max_delay + 1].flip(
            0).unsqueeze(-1)).squeeze(-1).sum(0)
    else: # case with kernels computed for all delays
        return (kernels[:max_delay + 1] @ dists[:max_delay + 1].flip(
            0).unsqueeze(-1)).squeeze(-1).sum(0)
...
def get_Ks_from_Us(Us: torch.Tensor, verbose: bool = True) -> torch.Tensor:
    ...
    for i in tqdm(range(timesteps - 1), desc='Kernels computed', disable=not verbose, leave=False):
        if i == 0: # base case
            Ks[i] = Us_safe[i + 1]
        else: # recursion
            Ks[i] = Us_safe[i + 1] - torch.bmm(Us_safe[1:i + 1], Ks[:i].flip(0)).sum(dim=0)
...
def extract_Us_dtnzgme(Ks: torch.Tensor, max_delay: int | None = None) -> torch.Tensor:
    ...
    if Ks.ndim != 3 or Ks.shape[1] != Ks.shape[2]:
        raise ValueError(f'Ks must be 3D and have square slices, got shape {tuple(Ks.shape)}.')
    integrals = torch.zeros((Ks.shape[0] + 1, Ks.shape[1], Ks.shape[2]), device=Ks.device, dtype=Ks.dtype)
    gme = NakajimaZwanzigGME()
    for i in range(Ks.shape[1]):
        ...
        integrals[:, :, i] = gme.integrate(
            Ks[:cutoff], Ks.shape[0], Ks.shape[1], Ks.device, initial_dist=initial_dist, verbose=False
        )
    return integrals
...
class EulerNZGME(GroundTruthNZGME):
    ...
    def get_k_ctgme(self, t: float) -> torch.Tensor:
        ...
        return self.macrostate_indicator.T @ self.trm @ QT_trm @ matrix_exp(QT_trm * t) @ self.p_micro_given_macro
    
    @torch.no_grad()
    def integrate(self, dt: float, n_steps: int,
                  kernels: torch.Tensor | None = None,
                  initial_dist: torch.Tensor | None = None,
                  verbose: bool = True) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if kernels is not None:
            ...
            kernels = torch.zeros(
                n_steps, self.n_macrostates, self.n_macrostates, dtype=torch.float64).to(self.device)
            for i in tqdm(range(n_steps), desc='Kernels computed', leave=False):
                kernels[i] = self.get_k_ctgme(i * dt)
            kernels[0] += self.v_ctgme / dt

        dists = super().integrate(kernels, n_steps, self.n_macrostates, self.device, dt=dt, initial_dist=initial_dist, verbose=verbose)
