#!/usr/bin/env python3
# GMEx/scripts/preprocess/CFTR.py
...
from gmex.utils import get_data_dir, save_metadata, STATE_LIST_FN, METADATA_FN
...
REF_FMT_FN = lambda suffix: f'Ref_{suffix}' # get format for reference-efficiency columns
STATE_FMT_FN = lambda suffix: f'State_{suffix}' # get format for state-assignment columns
FRET_FMT_FN = lambda suffix: f'Fret_{suffix}' # get format for raw FRET-efficiency columns
DONOR_FMT_FN = lambda suffix: f'Donor_{suffix}' # get format for donor-fluorescence columns
ACCEPTOR_FMT_FN = lambda suffix: f'Acceptor_{suffix}' # get format for acceptor-fluorescence columns
...
def _load_CFTR_FRET_raw(fn: str | Path, levels: List[float]) -> pd.DataFrame:
    ...
    idl_cols = [col for col in df.columns if pattern.match(col)]
    if not idl_cols:
        raise ValueError(f'No Idl_* columns found in {str(fn)}.')
    
    retained_cols = ['Time (s)']
    for idl_col in idl_cols:
        match = pattern.match(idl_col)
        suffix = match.group(1)
        fret_col = FRET_FMT_FN(suffix)
        donor_col = DONOR_FMT_FN(suffix)
        acceptor_col = ACCEPTOR_FMT_FN(suffix)
        missing_cols = [
            col for col in (fret_col, donor_col, acceptor_col)
    ...
    degen_cols = [
        col for col in df.filter(regex=IDL_FMT).columns if df[col].nunique(dropna=True) > max_levels
    ]
    if degen_cols:
        raise ValueError(
...
def _match_CFTR_FRET_levels(col: pd.Series, levels: List[float]) -> pd.Series:
    ...
    """
    pattern = re.compile(IDL_FMT)
    colname_match = pattern.match(col.name)
    if colname_match is None:
    ...
    if np.any(obs > 0.8): # this doesn't seem to occur
        raise ValueError('Trace has idealized efficiencies greater than 0.8, suggesting denaturation.')
    ...
    obs_match[obs <= col.iloc[-1]] = 0.0 # efficiencies less than or equal to the last assumed photobleached
    obs_match[obs < 0.05] = 0.0 # remaining efficiencies in z-test rejection region assumed photobleached
...
def _get_processed_CFTR_FRET_df(fn: str | Path, levels: List[float]) -> pd.DataFrame:
    ...
    derived_df = pd.concat(derived_cols, axis=1) if derived_cols else pd.DataFrame(index=df.index)
...
if __name__ == "__main__":
    ...
    for expt in EXPTS.keys():
        ...
        print(f'{expt.replace('_', ' ')} CFTR data saved to {str(outdir)}.')
