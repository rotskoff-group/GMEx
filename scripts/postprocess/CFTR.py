#!/usr/bin/env python3
# GMEx/scripts/postprocess/BMF1.py
...
from gmex.nakajima_zwanzig import get_Ks_from_Us, TransferMatrixNZGME
from gmex.utils import get_data_dir, get_results_dir, get_bootstrap_curve_CI
...
BOOTSTRAP_GS_FN = 'CFTR-L2-TCL-Gs-BS*.pt' # filename of saved bootstrap TCL-GME-DT propagators
BOOTSTRAP_US_FN = 'CFTR-L16-NZ-Us-BS*.pt' # filename of saved bootstrap transition matrices
...
if __name__ == "__main__":
    ...
    if SOLVENTX_CURVES_FN in os.listdir(RES_DIR):
        ...
        expt_solventx_mean = df['Mean high FRET occupancy (%)'].iloc[start_timestep:] / 100
        expt_solventx_err = (
            df['SD (%)'].iloc[start_timestep:] / 100 / np.sqrt(NPATCHES_SOLVENTX) * norm_CI_ppf_one_sided
        )

        # save processed experimental solvent-exchange curves
        solventx_curves['expt_means'] = expt_solventx_mean.to_list()
        solventx_curves['expt_lower'] = (expt_solventx_mean - expt_solventx_err).to_list()
        solventx_curves['expt_upper'] = (expt_solventx_mean + expt_solventx_err).to_list()
        ...
        _get_start_frac_high_FRET = lambda: float(norm.rvs(loc=solventx_start_mean, scale=solventx_start_err))
        nsteps_solventx_models = ceil((df['Time (s)'].max() - df['Time (s)'].iloc[start_timestep]) / DT)
        ...
        for i in range(len(bootstrap_WT_apo_Us_fns)):
            ...
            initial_dist = torch.tensor(
                [1.0 - p_high_0, p_high_0], dtype=torch.float64
            )
            Us = torch.load(bootstrap_WT_apo_dir / bootstrap_WT_apo_Us_fns[i], weights_only=True)
        ...
        solventx_curves['model_times'] = (np.arange(nsteps_solventx_models + 1) * DT).tolist()
        del (
    ...
    if RELAXATION_FN in os.listdir(RES_DIR):
        ...
        for expt in EXPTS: # iterate over experiments
            ...
            print(f"{expt.replace('_', ' ')} CFTR: {len(bootstrap_Us_fns)} bootstraps detected.")
            ...
            initial_dist = torch.tensor(
                [1.0, 0.0, 0.0] if expt in MUT_EXPTS else [1.0, 0.0], dtype=torch.float64
            )
            ...
            relax_curves[expt + '_msm_means'] = integrals_msm[:, -1].cpu().numpy().tolist()

            gme = TransferMatrixNZGME(Us[:N_LAGS_TRUNC + 2])
            _, integrals_gme = gme.integrate(
                NSTEPS_RELAX, initial_dist=initial_dist, verbose=False
            )
            relax_curves[expt + '_gme_means'] = integrals_gme[:, -1].cpu().numpy().tolist()
    ...
    if IRREVERSIBILITY_FN in os.listdir(RES_DIR):
        ...
        new_cols = [
            'MSM_EPR_mean (kB / s)', 'MSM_EPR_lower (kB / s)', 'MSM_EPR_upper (kB / s)',
            'GME_EPR_mean (kB / s)', 'GME_EPR_lower (kB / s)', 'GME_EPR_upper (kB / s)'
        ]
        ...
        for expt in EXPTS:
            ...
            bootstrap_Us_fns = _get_bootstrap_fns(BOOTSTRAP_US_FN, bootstrap_dir)
            assert len(bootstrap_Us_fns) == len(bootstrap_Gs_fns), 'unequal NZ and TCL bootstraps'
            print(f"{expt.replace('_', ' ')} CFTR: {len(bootstrap_Gs_fns)} bootstraps detected.")
            ...
            for i in tqdm(range(len(bootstrap_Gs_fns)), desc='Bootstraps', leave=False):
                U = torch.load(
                    bootstrap_dir / bootstrap_Us_fns[i], weights_only=True
                )[N_LAGS_TRUNC]
            ...
            U = torch.load(RES_DIR / expt / MAIN_RES_US_FN, weights_only=True)[N_LAGS_TRUNC]
            msm = MarkovChain(U, DT * N_LAGS_TRUNC, device='cpu')
            epr_df.at[expt, 'MSM_EPR_mean (kB / s)'] = float(msm.get_2nd_local_epr().item())

            Gs = torch.load(RES_DIR / expt / MAIN_RES_GS_FN, weights_only=True)
            assert Gs.shape[0] >= 3, 'Gs should have lags 0, 1, and 2.'
            gme = MarkovChain(Gs, DT * round(N_LAGS_TRUNC / 2), device='cpu')
            epr_df.at[expt, 'GME_EPR_mean (kB / s)'] = float(gme.get_3rd_local_epr().item())
    ...
    outer = fig.add_gridspec(
        nrows=4, ncols=1, height_ratios = [1.5, 1.75, 1.5, 1.5]
    )
    ...
    axes = [ax_r1_0, ax_r1_1, ax_r1_2, ax_r2_0, ax_r2_1, ax_r2_2, ax_r2_3, ax_r3_0, ax_r3_1, ax_r3_2, ax_r4_0, ax_r4_1, ax_r4_2]
    for a in axes:
    ...
    ax_r1_1.text(0.99, 0.03, 'WT', transform=ax_r1_1.transAxes, ha='right', va='bottom', fontsize=6)
    ...
    ax_r1_2.text(0.99, 0.03, 'Mutant (G551D)', transform=ax_r1_2.transAxes, ha='right', va='bottom', fontsize=6)
    ...
    fig.text(
        ...
        (
            min(pos.y0 for pos in r1_trace_positions) + max(pos.y1 for pos in r1_trace_positions)
        ) / 2,
        'FRET',
        rotation='vertical',
        ha='center',
    ...
    turnover_means = [
        epr_df.loc[expt]['ATP turnover mean (1 / min)'] / 60
    ...
    msm_epr_means = np.array([
        1e6 * epr_df.loc[expt]['MSM_EPR_mean (kB / s)']
        for expt in MUT_EXPTS
    ])
    msm_epr_lower = np.array([
        1e6 * epr_df.loc[expt]['MSM_EPR_lower (kB / s)']
        for expt in MUT_EXPTS
    ])
    msm_epr_upper = np.array([
        1e6 * epr_df.loc[expt]['MSM_EPR_upper (kB / s)']
        for expt in MUT_EXPTS
    ])
    ...
    ax_r2_2.set_ylabel(r'$\hat{\sigma}_{\mathrm{MSM}}$ ($\mu k_B$/s)', fontsize=8)

    gme_epr_color = 'C0'
    gme_epr_means = np.array([
        1e3 * epr_df.loc[expt]['GME_EPR_mean (kB / s)']
        for expt in MUT_EXPTS
    ])
    gme_epr_lower = np.array([
        1e3 * epr_df.loc[expt]['GME_EPR_lower (kB / s)']
        for expt in MUT_EXPTS
    ])
    gme_epr_upper = np.array([
        1e3 * epr_df.loc[expt]['GME_EPR_upper (kB / s)']
        for expt in MUT_EXPTS
    ])
    ...
    ax_r2_3.set_ylabel(r'$\hat{\sigma}_{\mathrm{GME}}$ (m$k_B$/s)', fontsize=8, labelpad=0)
    ...
    fig.legend(
        handles=[
            ...
            Patch(facecolor='white', edgecolor='k', linewidth=0.5, hatch='////', label='GLPG1837'),
        ],
        loc='upper center',
    ...
    ax_r3_0.axvline(x=N_LAGS_TRUNC * DT, linestyle='--', label='Cutoff', color='k', linewidth=0.5)
    ax_r3_0.set_xlim(plot_Ks_lagtimes[0], plot_Ks_lagtimes[-1])
    ax_r3_0.set_ylim(-1e-2, 1e-2)
    ax_r3_0.text(0.96, 0.1, 'Apo', transform=ax_r3_0.transAxes, ha='right', va='bottom', fontsize=6)
    ax_r3_0.legend(
        handles=[
    ...
    ax_r3_1.axvline(x=N_LAGS_TRUNC * DT, linestyle='--', label='Cutoff', color='k', linewidth=0.5)
    ax_r3_1.set_xlim(plot_Ks_lagtimes[0], plot_Ks_lagtimes[-1])
    ax_r3_1.set_ylim(-1e-2, 1e-2)
    ax_r3_1.text(0.96, 0.1, r'GLPG1837', transform=ax_r3_1.transAxes, ha='right', va='bottom', fontsize=6)
    ...
    ### panel (e) ###
    ax_r3_2.plot(
        solventx_curves['expt_times'], solventx_curves['expt_means'], c='k'
    )
    ax_r3_2.fill_between(
        solventx_curves['expt_times'], solventx_curves['expt_lower'], solventx_curves['expt_upper'], color='k', alpha=0.25
    )
    ax_r3_2.plot(
        solventx_curves['model_times'], solventx_curves['msm_means'], c='C1'
    )
    ax_r3_2.fill_between(
        solventx_curves['model_times'], solventx_curves['msm_lower'], solventx_curves['msm_upper'], color='C1', alpha=0.25
    )
    ax_r3_2.plot(
        solventx_curves['model_times'], solventx_curves['gme_means'], c='C0'
    )
    ax_r3_2.fill_between(
        solventx_curves['model_times'], solventx_curves['gme_lower'], solventx_curves['gme_upper'], color='C0', alpha=0.25
    )
    ...
    # WT models
    ax_r4_0.plot(
        relax_times, relax_curves['WT_3mM_ATP_msm_means'], c='C1'
    )
    ax_r4_0.fill_between(
        relax_times, relax_curves['WT_3mM_ATP_msm_lower'], relax_curves['WT_3mM_ATP_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_0.plot(
        relax_times, relax_curves['WT_3mM_ATP_gme_means'], c='C0'
    )
    ax_r4_0.fill_between(
        relax_times, relax_curves['WT_3mM_ATP_gme_lower'], relax_curves['WT_3mM_ATP_gme_upper'], color='C0', alpha=0.25
    )
    ax_r4_0.plot(
        relax_times, relax_curves['WT_3mM_ATP_10uM_GLPG1837_msm_means'], c='C1', linestyle='-.'
    )
    ax_r4_0.fill_between(
        relax_times, relax_curves['WT_3mM_ATP_10uM_GLPG1837_msm_lower'], relax_curves['WT_3mM_ATP_10uM_GLPG1837_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_0.plot(
        relax_times, relax_curves['WT_3mM_ATP_10uM_GLPG1837_gme_means'], c='C0', linestyle='-.'
    )
    ax_r4_0.fill_between(
        relax_times, relax_curves['WT_3mM_ATP_10uM_GLPG1837_gme_lower'], relax_curves['WT_3mM_ATP_10uM_GLPG1837_gme_upper'], color='C0', alpha=0.25
    )
    ...
    ax_r4_0.text(0.98, 0.02, 'WT', transform=ax_r4_0.transAxes, ha='right', va='bottom', fontsize=6)

    # G551D models
    ax_r4_1.plot(
        relax_times, relax_curves['G551D_3mM_ATP_msm_means'], c='C1'
    )
    ax_r4_1.fill_between(
        relax_times, relax_curves['G551D_3mM_ATP_msm_lower'], relax_curves['G551D_3mM_ATP_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_1.plot(
        relax_times, relax_curves['G551D_3mM_ATP_gme_means'], c='C0'
    )
    ax_r4_1.fill_between(
        relax_times, relax_curves['G551D_3mM_ATP_gme_lower'], relax_curves['G551D_3mM_ATP_gme_upper'], color='C0', alpha=0.25
    )
    ax_r4_1.plot(
        relax_times, relax_curves['G551D_3mM_ATP_10uM_GLPG1837_msm_means'], c='C1', linestyle='-.'
    )
    ax_r4_1.fill_between(
        relax_times, relax_curves['G551D_3mM_ATP_10uM_GLPG1837_msm_lower'], relax_curves['G551D_3mM_ATP_10uM_GLPG1837_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_1.plot(
        relax_times, relax_curves['G551D_3mM_ATP_10uM_GLPG1837_gme_means'], c='C0', linestyle='-.'
    )
    ax_r4_1.fill_between(
        relax_times, relax_curves['G551D_3mM_ATP_10uM_GLPG1837_gme_lower'], relax_curves['G551D_3mM_ATP_10uM_GLPG1837_gme_upper'], color='C0', alpha=0.25
    )
    ax_r4_1.set_xlim(relax_times[0], relax_times[-1])
    ax_r4_1.set_xlabel('Time (s)', fontsize=8)
    ax_r4_1.text(0.98, 0.02, 'G551D', transform=ax_r4_1.transAxes, ha='right', va='bottom', fontsize=6)
    ax_r4_1.legend(
        handles=[
            (
                Patch(facecolor='C1', edgecolor='none', alpha=0.25),
    ...
    # L927P models
    ax_r4_2.plot(
        relax_times, relax_curves['L927P_3mM_ATP_msm_means'], c='C1'
    )
    ax_r4_2.fill_between(
        relax_times, relax_curves['L927P_3mM_ATP_msm_lower'], relax_curves['L927P_3mM_ATP_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_2.plot(
        relax_times, relax_curves['L927P_3mM_ATP_gme_means'], c='C0'
    )
    ax_r4_2.fill_between(
        relax_times, relax_curves['L927P_3mM_ATP_gme_lower'], relax_curves['L927P_3mM_ATP_gme_upper'], color='C0', alpha=0.25
    )
    ax_r4_2.plot(
        relax_times, relax_curves['L927P_3mM_ATP_10uM_GLPG1837_msm_means'], c='C1', linestyle='-.'
    )
    ax_r4_2.fill_between(
        relax_times, relax_curves['L927P_3mM_ATP_10uM_GLPG1837_msm_lower'], relax_curves['L927P_3mM_ATP_10uM_GLPG1837_msm_upper'], color='C1', alpha=0.25
    )
    ax_r4_2.plot(
        relax_times, relax_curves['L927P_3mM_ATP_10uM_GLPG1837_gme_means'], c='C0', linestyle='-.'
    )
    ax_r4_2.fill_between(
        relax_times, relax_curves['L927P_3mM_ATP_10uM_GLPG1837_gme_lower'], relax_curves['L927P_3mM_ATP_10uM_GLPG1837_gme_upper'], color='C0', alpha=0.25
    )
    ax_r4_2.set_xlim(relax_times[0], relax_times[-1])
    ax_r4_2.set_xlabel('Time (s)', fontsize=8)
    ax_r4_2.text(0.98, 0.02, 'L927P', transform=ax_r4_2.transAxes, ha='right', va='bottom', fontsize=6)
    ...
    plt.savefig(RES_DIR / FIG_FN, bbox_inches='tight')
