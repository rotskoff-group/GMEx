#!/usr/bin/env python3
# GMEx/scripts/postprocess/HP35.py
...
from gmex.utils import get_data_dir, load_times_macrostates, get_results_dir, fpts_from_traj, get_bootstrap_curve_CI
...
FIG_FN = 'HP35-fig-no-ribbon.pdf' # filename for HP35 figure without ribbon in panel (a)

RESAMPLING = 5 # we want a 1 ns timestep and MD trajectory is sampled every 200 ps
TAU_21_LAB_MEAN = 730 # mean experimental unfolded -> near-native first-passage time in ns
...
def _get_bootstrap_fns(template: str) -> List[str]:
    ...
    return sorted(
        name for name in os.listdir(BOOTSTRAP_DIR)
...
if __name__ == "__main__":
    ...
    if FPT_FN in os.listdir(RES_DIR) and MFPT_FN in os.listdir(RES_DIR):
        ...
        bootstrap_fpt_buffer = np.empty((len(bootstrap_fns), N_TRAJ_FPT), dtype=np.uint32)

        # get GME 2 -> 1 FPT CIs
        for i in tqdm(range(len(bootstrap_fns)), desc='GME 2 -> 1 FPT CIs', leave=False):
            Gs = torch.load(BOOTSTRAP_DIR / bootstrap_fns[i], weights_only=True)
            gme = MarkovChain(Gs[:N_LAGS_TRUNC + 1], 1.0, device=device)
            bootstrap_fpt_buffer[i] = gme.first_passage_times(
                2, 1, n_trajs=N_TRAJ_FPT
            ).cpu().numpy().astype('uint32')

        mfpts_gme_21 = np.mean(bootstrap_fpt_buffer, axis=1)
        mfpts_gme_21_lower, mfpts_gme_21_upper = get_bootstrap_curve_CI(mfpts_gme_21[:, None], CI_PCT)
        ...
        fpts_gme_21_lower, fpts_gme_21_upper = get_bootstrap_curve_CI(bootstrap_fpt_buffer, CI_PCT)
        fpts['gme_21_lower'] = fpts_gme_21_lower.tolist()
        fpts['gme_21_upper'] = fpts_gme_21_upper.tolist()

        # get GME 1 -> 0 FPT CIs
        for i in tqdm(range(len(bootstrap_fns)), desc='GME 1 -> 0 FPT CIs', leave=False):
            Gs = torch.load(BOOTSTRAP_DIR / bootstrap_fns[i], weights_only=True)
            gme = MarkovChain(Gs[:N_LAGS_TRUNC + 1], 1.0, device=device)
            bootstrap_fpt_buffer[i] = gme.first_passage_times(
                1, 0, n_trajs=N_TRAJ_FPT
            ).cpu().numpy().astype('uint32')

        mfpts_gme_10 = np.mean(bootstrap_fpt_buffer, axis=1)
        mfpts_gme_10_lower, mfpts_gme_10_upper = get_bootstrap_curve_CI(mfpts_gme_10[:, None], CI_PCT)
        ...
        fpts_gme_10_lower, fpts_gme_10_upper = get_bootstrap_curve_CI(bootstrap_fpt_buffer, CI_PCT)
        fpts['gme_10_lower'] = fpts_gme_10_lower.tolist()
        fpts['gme_10_upper'] = fpts_gme_10_upper.tolist()

        # get MSM 2 -> 1 FPT CIs
        for i in tqdm(range(len(bootstrap_fns)), desc='MSM 2 -> 1 FPT CIs', leave=False):
            Gs = torch.load(BOOTSTRAP_DIR / bootstrap_fns[i], weights_only=True)
            msm = MarkovChain(Gs[1], 1.0, device=device)
            bootstrap_fpt_buffer[i] = msm.first_passage_times(
                2, 1, n_trajs=N_TRAJ_FPT
            ).cpu().numpy().astype('uint32')

        mfpts_msm_21 = np.mean(bootstrap_fpt_buffer, axis=1)
        mfpts_msm_21_lower, mfpts_msm_21_upper = get_bootstrap_curve_CI(mfpts_msm_21[:, None], CI_PCT)
        ...
        fpts_msm_21_lower, fpts_msm_21_upper = get_bootstrap_curve_CI(bootstrap_fpt_buffer, CI_PCT)
        fpts['msm_21_lower'] = fpts_msm_21_lower.tolist()
        fpts['msm_21_upper'] = fpts_msm_21_upper.tolist()

        # get MSM 1 -> 0 FPT CIs
        for i in tqdm(range(len(bootstrap_fns)), desc='MSM 1 -> 0 FPT CIs', leave=False):
            Gs = torch.load(BOOTSTRAP_DIR / bootstrap_fns[i], weights_only=True)
            msm = MarkovChain(Gs[1], 1.0, device=device)
            bootstrap_fpt_buffer[i] = msm.first_passage_times(
                1, 0, n_trajs=N_TRAJ_FPT
        ...
        mfpts_msm_10_lower, mfpts_msm_10_upper = get_bootstrap_curve_CI(mfpts_msm_10[:, None], CI_PCT)
        ...
        fpts_msm_10_lower, fpts_msm_10_upper = get_bootstrap_curve_CI(bootstrap_fpt_buffer, CI_PCT)
        ...
        fpts['lab_21_lower'] = expon.ppf(
            pctiles, scale=TAU_21_LAB_MEAN - norm_CI_ppf_one_sided * TAU_21_LAB_SE).tolist()
        fpts['lab_21_upper'] = expon.ppf(
            pctiles, scale=TAU_21_LAB_MEAN + norm_CI_ppf_one_sided * TAU_21_LAB_SE).tolist()
        fpts['lab_10'] = expon.ppf(pctiles, scale=TAU_10_LAB).tolist()

        student_CI_ppf_one_sided = lambda n: -student.ppf(0.5 - CI_PCT / 2.0, n - 1).item()
    ...
    def broken_y(values, low=(0.0, 0.05), high=(0.95, 1.0), gap=0.08):
        ...
        mapped[upper] = upper_start + (values[upper] - high[0]) / (high[1] - high[0]) * lower_height
        return mapped

    Gs_y = Gs.reshape(Gs.shape[0], -1)[1:].cpu().numpy()
    ax[0, 1].plot(
        torch.arange(1, Gs.shape[0]).cpu(), broken_y(Gs_y),
    ...
    for y in (0.5 - 0.08 / 2, 0.5 + 0.08 / 2):
        ax[0, 1].plot(
            (-0.015, 0.015), (y - 0.015, y + 0.015),
            transform=ax[0, 1].transAxes,
            color='k', linewidth=0.5, clip_on=False
        )
    ax[0, 1].set_ylabel(r'$\hat{G}^{(n)}_{ij}$', fontsize=8)
    ax[0, 1].set_xlabel(r'Lagtime $n\tau$ (ns)', fontsize=8)
    ax[0, 1].axvline(x=N_LAGS_TRUNC, linestyle='--', label='Cutoff', color='k', linewidth=0.5)
    ...
    ax[1, 0].fill_betweenx(
        pctiles, fpts['msm_21_lower'], fpts['msm_21_upper'],
        color='C1', alpha=0.25, label='_nolegend_'
    )
    ax[1, 0].fill_betweenx(
        pctiles, fpts['gme_21_lower'], fpts['gme_21_upper'],
        color='C0', alpha=0.25, label='_nolegend_'
    )
    ax[1, 0].fill_betweenx(
        pctiles, fpts['lab_21_lower'], fpts['lab_21_upper'],
        color='k', alpha=0.25, label='_nolegend_'
    )
    ...
    lab_10, = ax[1, 1].plot(fpts['lab_10'], pctiles,
                            color='k', linestyle=':', label='$T$-jump\n(approx.)')
    
    ax[1, 1].fill_betweenx(
        pctiles, fpts['msm_10_lower'], fpts['msm_10_upper'],
        color='C1', alpha=0.25, label='_nolegend_'
    )
    ax[1, 1].fill_betweenx(
        pctiles, fpts['gme_10_lower'], fpts['gme_10_upper'],
        color='C0', alpha=0.25, label='_nolegend_'
    )
    ...
    panel_label_x_nudge = -0.01  # figure-coordinate units; more negative moves labels left
    for label, x_key, y_key in [
        ...
        fig.text(
            panel_label_x[x_key] + panel_label_x_nudge, panel_label_y[y_key], label,
            ha='center', va='center',
            fontsize=8
        )
    ...
    print(f'HP35 figure saved to {str(RES_DIR / FIG_FN)}')
