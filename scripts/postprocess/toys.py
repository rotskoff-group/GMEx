...
def _add_filled_arrowhead(
    fig, tip, direction, color="k", zorder=4, head_length=0.06, head_width=0.055
):
...
class _FilledHeadArrow(FancyArrowPatch):
    """Identify legend arrows rendered with a custom filled head."""
...
class _HandlerArrow(HandlerBase):
    ...
    def create_artists(
        ...
        if isinstance(orig_handle, _FilledHeadArrow):
...
def _load_toy_summary(toyname: str):
    ...
    Ks_analytic = torch.load(DATA_DIR / toyname / TRUE_KS_FN, weights_only=True).to(
        "cpu"
    )
    ...
    for i in range(len(N_STEPS)):
        ...
        for j in range(len(rep_U_fns)):
            ...
            Us_diff = Us_true[1 : N_LAGS_TRUNC + 1] - Us_est[1 : N_LAGS_TRUNC + 1]
            rep_U_buffer[j] = torch.linalg.matrix_norm(Us_diff, ord=2)

            Ks_est = get_Ks_from_Us(Us_est, verbose=False)
            Ks_diff = Ks_analytic[:N_LAGS_TRUNC] - Ks_est[:N_LAGS_TRUNC]
        ...
        for j in range(len(rep_G_fns)):
            ...
            Gs_diff = Gs_true[1 : N_LAGS_TRUNC + 1] - Gs_est[1 : N_LAGS_TRUNC + 1]
...
def _draw_compact_diagrams(ax: Axes):
    ...
    for dot_y, c01, c12 in kernel_terms:
        scatter_artists.append(
            ax.scatter(dot_x, [dot_y] * 3, s=8, color="black", zorder=3, clip_on=False)
        )
        if c01:
            (line,) = ax.plot(
                [dot_x[0], dot_x[1]],
    ...
    time_label = ax.text(
        dot_x.mean(), 4 + 1 / 6, "Time", fontsize=8, ha="center", va="bottom"
    )
    k_label = ax.text(
        (dot_x[1] + dot_x[2]) / 2,
        1.50,
...
def _update_compact_diagram_xcoords(diagram_artists: dict, dot_x: np.ndarray) -> None:
    ...
    diagram_artists["bottom_labels"][0].set_position(
        ((dot_x[0] + dot_x[1]) / 2, bottom_label_y0)
    )
...
def _update_compact_diagram_ycoords(diagram_artists: dict, dot_y: np.ndarray) -> None:
    ...
    diagram_artists["k_label"].set_position(
        ((dot_x[1] + dot_x[2]) / 2, (dot_y[1] + dot_y[2]) / 2)
    )
    ...
    diagram_artists["time_arrow"].set_positions(
        (dot_x[-1] + 0.25, arrow_y), (dot_x[0] - 0.25, arrow_y)
    )


def _match_compact_diagram_arrow_to_time_label(
    fig, ax: Axes, diagram_artists: dict
) -> None:
    """Make the top diagram arrow the rendered width of the Time label."""
    time_bbox = diagram_artists["time_label"].get_window_extent()
    ...
    diagram_artists["time_arrow"].set_positions(
        (time_x + half_width, arrow_y), (time_x - half_width, arrow_y)
    )
...
def _align_axes_text_right_to_fig_x(fig, ax: Axes, text, target_x: float) -> None:
    ...
    inv_fig = fig.transFigure.inverted()
    bbox = text.get_window_extent().transformed(inv_fig)
    text_x, text_y = text.get_position()
    text.set_position(
        (text_x + _data_dx_from_fig_dx(fig, ax, target_x - bbox.x1), text_y)
    )
...
if __name__ == "__main__":
    ...
    gs_middle = gs[1].subgridspec(
        1, 3, width_ratios=[10, 9, 10], wspace=middle_column_wspace
    )
    gs_bottom = gs[3].subgridspec(
    ...
    for state_i, state_j, outside, label_offset, label in outer_harpoons:
        ...
        rate_label_pos = (harpoon_start + harpoon_end) / 2 + label_offset * outside
        rate_label = ax_top_left.text(
            float(rate_label_pos[0]),
            float(rate_label_pos[1]),
            label,
            fontsize=8,
            ha="center",
            va="center",
    ...
    assert top_outer_rate_label is not None
    assert left_outer_rate_label is not None
    assert bottom_outer_rate_label is not None
    ...
    for state_i, state_j, inside, label_offset, label in inner_harpoons:
        ...
        inner_harpoon_label_pos = (
            harpoon_start + harpoon_end
        ) / 2 + label_offset * inside
        ax_top_left.text(
            float(inner_harpoon_label_pos[0]),
            float(inner_harpoon_label_pos[1]),
            label,
            fontsize=8,
            ha="center",
            va="center",
    ...
    ax_top_right.plot(trace_window, np.asarray(trace)[trace_window], c="k")
    ...
    if top_outer_rate_label is not None and bottom_outer_rate_label is not None:
        top_rate_bbox = top_outer_rate_label.get_window_extent().transformed(inv_fig)
        bottom_rate_bbox = bottom_outer_rate_label.get_window_extent().transformed(
            inv_fig
        )
        top_macro_bbox = max(
            (
                ticklabel.get_window_extent().transformed(inv_fig)
                for ticklabel in ax_top_right.get_yticklabels()
                if ticklabel.get_visible()
            ),
            key=lambda bbox: (bbox.y0 + bbox.y1) / 2,
        )
        xlabel_bbox = ax_top_right.xaxis.label.get_window_extent().transformed(inv_fig)
        ...
        if rate_span > 0 and target_span > 0:
            ...
            ax_top_left.set_position((top_left_pos.x0, new_y0, new_width, new_height))
            fig.draw_without_rendering()

    top_right_ticklabel_right = max(
        ticklabel.get_window_extent().transformed(inv_fig).x1
        for ticklabel in ax_top_right.get_yticklabels()
        if ticklabel.get_visible()
    )
    ...
    ### panels (c), (d), (e) ###
    ax_middle_left.plot(
        plot_lags, Us_true.reshape(Us_true.shape[0], -1).cpu(), c="C1", linewidth=0.5
    )
    ...
    ax_middle_right.plot(
        plot_lags, Gs_true.reshape(Gs_true.shape[0], -1).cpu(), c="C0", linewidth=0.5
    )
    ...
    if left_outer_rate_label is not None:
        left_rate_bbox = left_outer_rate_label.get_window_extent().transformed(inv_fig)
        target_ticklabels = [
            ticklabel
            for ticklabel in ax_middle_left.get_yticklabels()
            if ticklabel.get_visible()
        ]
    ...
    ax_top_right.set_position(
        (trace_x0, trace_pos.y0, trace_x1 - trace_x0, trace_pos.height)
    )
    fig.draw_without_rendering()
    top_right_ticklabel_right = max(
        ticklabel.get_window_extent().transformed(inv_fig).x1
    ...
    ax_top_diagram = fig.add_axes(
        (diagram_x0, top_right_pos.y0, diagram_width, top_right_pos.height)
    )
    diagram_artists = _draw_compact_diagrams(ax_top_diagram)
    fig.draw_without_rendering()
    trace_xlabel_bbox = ax_top_right.xaxis.label.get_window_extent().transformed(
        inv_fig
    )
    trace_xlabel_y = (trace_xlabel_bbox.y0 + trace_xlabel_bbox.y1) / 2
    panel_a_y_for_diagram = (
        top_outer_rate_label.get_window_extent().transformed(inv_fig).y0
        + top_outer_rate_label.get_window_extent().transformed(inv_fig).y1
    ) / 2
    diagram_pos = ax_top_diagram.get_position()
    diagram_bottom_label_y = np.mean(
        [
            (
                label.get_window_extent().transformed(inv_fig).y0
                + label.get_window_extent().transformed(inv_fig).y1
            )
            / 2
            for label in diagram_artists["bottom_labels"]
        ]
    )
    diagram_time_label_bbox = (
        diagram_artists["time_label"].get_window_extent().transformed(inv_fig)
    )
    ...
    diagram_height = (panel_a_y_for_diagram - trace_xlabel_y) / (
    ...
    ax_top_diagram.set_position(
        (diagram_pos.x0, diagram_y0, diagram_pos.width, diagram_height)
    )
    ...
    longest_trace_tick_bbox = max(trace_tick_bboxes, key=lambda bbox: bbox.x1 - bbox.x0)
    trace_pos = ax_top_right.get_position()
    ...
    fig.draw_without_rendering()
    inv_fig = fig.transFigure.inverted()
    cycle_bbox = ax_top_left.get_window_extent().transformed(inv_fig)
    center_bbox = ax_center.get_window_extent().transformed(inv_fig)
    ...
    fig.draw_without_rendering()
    label_bbox = heptagon_label.get_window_extent().transformed(inv_fig)
    label_height = label_bbox.y1 - label_bbox.y0
    middle_xlabel_bbox = ax_middle_left.xaxis.label.get_window_extent().transformed(
        inv_fig
    )
    heptagon_center = (
        heptagon_center[0] - 0.035,
        (
            middle_xlabel_bbox.y0
            + ax_bottom_left.get_position().y1
            - heptagon_y_offsets.max()
            - heptagon_y_offsets.min()
            - heptagon_label_gap
            - label_height
        )
        / 2,
    ...
    ellipse_ax = fig.add_axes(
        (ellipse_x0, ellipse_y0, ellipse_width, ellipse_height), zorder=1
    )
    ellipse_ax.patch.set_alpha(0)
    ellipse_ax.axis("off")
    gradient_n = 256
    xx, yy = np.meshgrid(np.linspace(0, 1, gradient_n), np.linspace(0, 1, gradient_n))
    ...
    gradient = np.clip(
        0.8 * distance_from_gradient_center / distance_to_right_vertex, 0, 0.8
    )

    heptagon_anchor = np.array(heptagon_center)
    ellipse_center_fig = np.array(
        [(ellipse_x0 + ellipse_x1) / 2, (ellipse_y0 + ellipse_y1) / 2]
    )
    ellipse_center_in = np.array(
        [ellipse_center_fig[0] * fig_width, ellipse_center_fig[1] * fig_height]
    )
    ellipse_a = ellipse_width_in / 2
    ellipse_b = ellipse_height_in / 2
    heptagon_anchor_in = np.array(
        [heptagon_anchor[0] * fig_width, heptagon_anchor[1] * fig_height]
    )
    ...
    ellipse_top_point = np.array([ellipse_center_fig[0], ellipse_y1])
    fig.patches.append(
        Polygon(
    ...
    ellipse_clip = Ellipse(
        (0.5, 0.5), width=1.0, height=1.0, transform=ellipse_ax.transAxes
    )
    ...
    fig.text(
        *eta_inf_pos, r"$\eta\to\infty$", fontsize=8, ha="right", va="center", color="k"
    )
    gradient_center_fig = (
    ...
    eta_arrow_base = _add_filled_arrowhead(
        fig, eta_arrow_tip, eta_arrow_direction, color="C6", zorder=4.1
    )
    fig.lines.append(
        Line2D(
    ...
    fig.patches.append(
        FancyArrowPatch(
            ...
            (
                float(intermediate_dot_positions[0][0]),
                float(intermediate_dot_positions[0][1]),
            ),
            transform=fig.transFigure,
            arrowstyle="->",
            mutation_scale=8,
    ...
    dotted_connector_t = (
        intermediate_dot_positions[1][0] - eta_zero_edge_pos[0]
    ) / dot_to_eta[0]
    ...
    fig.patches.append(
        FancyArrowPatch(
            (
                float(dotted_connector_start[0]),
                float(dotted_connector_start[1]),
            ),
            (
                float(intermediate_dot_positions[1][0]),
                float(intermediate_dot_positions[1][1]),
            ),
            transform=fig.transFigure,
            arrowstyle="->",
            mutation_scale=8,
    ...
    fig.draw_without_rendering()
    inv_fig = fig.transFigure.inverted()
    eta_zero_bbox = eta_zero_label.get_window_extent().transformed(inv_fig)
    ...
    fig.draw_without_rendering()
    title_bbox = colorbar_title.get_window_extent().transformed(inv_fig)
    ...
    colorbar_ax = fig.add_axes(
        (colorbar_x0, colorbar_y0, colorbar_x1 - colorbar_x0, colorbar_height), zorder=2
    )
    colorbar_ax.imshow(
        np.linspace(0, 0.8, 256)[None, :], cmap="GnBu", aspect="auto", vmin=0, vmax=1
    )
    ...
    legend_handles = [
        ...
        FancyArrowPatch(
            (0, 0), (1, 0), arrowstyle="->", color="k", linewidth=0.5, linestyle="-"
        ),
        FancyArrowPatch(
            (0, 0), (1, 0), arrowstyle="->", color="k", linewidth=0.5, linestyle=":"
        ),
    ...
    ax_bottom_left.errorbar(
        N_STEPS,
        err_U_means[0],
        student_CI_ppf_one_sided * err_U_stderrs[0],
        color="C1",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_bottom_left.errorbar(
        N_STEPS,
        err_U_means[1],
        student_CI_ppf_one_sided * err_U_stderrs[1],
        color="C1",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_bottom_left.errorbar(
        N_STEPS,
        err_U_means[2],
        student_CI_ppf_one_sided * err_U_stderrs[2],
        color="C1",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    ax_bottom_center.errorbar(
        N_STEPS,
        err_K_means[0],
        student_CI_ppf_one_sided * err_K_stderrs[0],
        color="C0",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_bottom_center.errorbar(
        N_STEPS,
        err_K_means[1],
        student_CI_ppf_one_sided * err_K_stderrs[1],
        color="C0",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_bottom_center.errorbar(
        N_STEPS,
        err_K_means[2],
        student_CI_ppf_one_sided * err_K_stderrs[2],
        color="C0",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    ax_bottom_right.errorbar(
        N_STEPS,
        err_G_means[0],
        student_CI_ppf_one_sided * err_G_stderrs[0],
        color="C0",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_bottom_right.errorbar(
        N_STEPS,
        err_G_means[1],
        student_CI_ppf_one_sided * err_G_stderrs[1],
        color="C0",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_bottom_right.errorbar(
        N_STEPS,
        err_G_means[2],
        student_CI_ppf_one_sided * err_G_stderrs[2],
        color="C0",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    def _ylabel_x(ax):
        bbox = ax.yaxis.label.get_window_extent().transformed(inv_fig)
    ...
    for bottom_ax, middle_ax in [
        ...
        bottom_ax.yaxis.set_label_coords(
            _ylabel_x(middle_ax), bottom_ax.get_position().y0, transform=fig.transFigure
        )
        bottom_ax.yaxis.label.set_horizontalalignment("left")
        bottom_ax.yaxis.label.set_verticalalignment("center")

    ### panel annotations ###
    panel_label_style = {"fontsize": 8, "ha": "center", "va": "center"}
    ...
    for row_y, row_label in [
        ...
        row_fig_y = inv_fig.transform(ax_top_diagram.transData.transform((0, row_y)))[1]
        diagram_row_labels.append(
            fig.text(
                diagram_row_label_x,
                row_fig_y,
                row_label,
    ...
    for row_label in diagram_row_labels:
        row_label_bbox = row_label.get_window_extent().transformed(inv_fig)
        row_label_x, row_label_y = row_label.get_position()
        row_label.set_position(
            (row_label_x + diagram_row_label_x - row_label_bbox.x0, row_label_y)
        )
    fig.draw_without_rendering()
    macrostate_ylabel_bbox = macrostate_ylabel.get_window_extent().transformed(inv_fig)
    top_row_label_bbox = diagram_row_labels[0].get_window_extent().transformed(inv_fig)
    ...
    u_row_label_right = max(
        row_label.get_window_extent().transformed(inv_fig).x1
        for row_label in diagram_row_labels
    )
    ...
    fig.draw_without_rendering()
    k0_bbox = k0_label.get_window_extent().transformed(inv_fig)
    ...
    macrostate_ylabel.set_position(
        (macrostate_ylabel.get_position()[0], upper_u1_row_y)
    )
    fig.draw_without_rendering()
    _match_compact_diagram_arrow_to_time_label(fig, ax_top_diagram, diagram_artists)

    panel_a_x = _ylabel_x(ax_middle_left)
    panel_a_y = _bbox_center(
        top_outer_rate_label.get_window_extent().transformed(inv_fig)
    )[1]
    ...
    fig.text(panel_a_x, panel_a_y, "(a)", fontdict=panel_label_style)
    fig.text(panel_b_x, panel_b_y, "(b)", fontdict=panel_label_style)
    panel_c_label = fig.text(
        panel_c_x, panel_c_y, "(c)", fontsize=8, ha="left", va="center"
    )
    fig.draw_without_rendering()
    panel_c_bbox = panel_c_label.get_window_extent().transformed(inv_fig)
    ...
    fig.text(panel_d_x, panel_d_y, "(d)", fontdict=panel_label_style)
    fig.text(panel_e_x, panel_e_y, "(e)", fontdict=panel_label_style)
    fig.text(panel_f_x, panel_f_y, "(f)", fontdict=panel_label_style)
    fig.text(panel_g_x, panel_g_y, "(g)", fontdict=panel_label_style)
    fig.text(panel_h_x, panel_h_y, "(h)", fontdict=panel_label_style)
    fig.text(panel_i_x, panel_i_y, "(i)", fontdict=panel_label_style)
    fig.text(panel_j_x, panel_j_y, "(j)", fontdict=panel_label_style)

    fig.savefig(RES_DIR / SCHEME_FN, bbox_inches="tight")
    ...
    ax_adv_middle_right = fig_adv.add_subplot(
        gs_adv_middle[0, 2], sharey=ax_adv_middle_left
    )
    ax_adv_bottom_left = fig_adv.add_subplot(gs_adv_bottom[0, 0])
    ax_adv_bottom_center = fig_adv.add_subplot(
        gs_adv_bottom[0, 1], sharey=ax_adv_bottom_left
    )
    ax_adv_bottom_right = fig_adv.add_subplot(
        gs_adv_bottom[0, 2], sharey=ax_adv_bottom_left
    )
    ...
    axes_adv = [
        ...
        ax_adv_top_right,
        ax_adv_middle_left,
        ax_adv_middle_center,
        ax_adv_middle_right,
        ax_adv_bottom_left,
        ax_adv_bottom_center,
        ax_adv_bottom_right,
    ]
    for a in axes_adv:
        a.tick_params(axis="both", which="both", labelsize=8)
    ...
    for state_i, state_j, outside, label_offset, _ in outer_harpoons:
        ...
        adv_rate_label_pos = (harpoon_start + harpoon_end) / 2 + label_offset * outside
        rate_label = ax_adv_top_left.text(
            float(adv_rate_label_pos[0]),
            float(adv_rate_label_pos[1]),
            "1",
            fontsize=8,
            ha="center",
            va="center",
    ...
    assert top_outer_rate_label_adv is not None
    assert left_outer_rate_label_adv is not None
    assert bottom_outer_rate_label_adv is not None
    ...
    for state_i, state_j, inside, label_offset, _ in inner_harpoons:
        ...
        adv_inner_harpoon_label_pos = (
            harpoon_start + harpoon_end
        ) / 2 + label_offset * inside
        ax_adv_top_left.text(
            float(adv_inner_harpoon_label_pos[0]),
            float(adv_inner_harpoon_label_pos[1]),
            r"$1/2$",
            fontsize=8,
            ha="center",
            va="center",
    ...
    fig_adv.draw_without_rendering()
    inv_fig_adv = fig_adv.transFigure.inverted()
    top_rate_bbox_adv = top_outer_rate_label_adv.get_window_extent().transformed(
        inv_fig_adv
    )
    bottom_rate_bbox_adv = bottom_outer_rate_label_adv.get_window_extent().transformed(
        inv_fig_adv
    )
    top_macro_bbox_adv = max(
        (
            ticklabel.get_window_extent().transformed(inv_fig_adv)
            for ticklabel in ax_adv_top_right.get_yticklabels()
            if ticklabel.get_visible()
        ),
        key=lambda bbox: (bbox.y0 + bbox.y1) / 2,
    )
    xlabel_bbox_adv = ax_adv_top_right.xaxis.label.get_window_extent().transformed(
        inv_fig_adv
    )
    ...
    if rate_span_adv > 0 and target_span_adv > 0:
        top_rate_rel_adv = (
            top_rate_y_adv - top_left_pos_adv.y0
        ) / top_left_pos_adv.height
        bottom_rate_rel_adv = (
            bottom_rate_y_adv - top_left_pos_adv.y0
        ) / top_left_pos_adv.height
        ...
        ax_adv_top_left.set_position(
            (top_left_pos_adv.x0, new_y0_adv, new_width_adv, new_height_adv)
        )
        fig_adv.draw_without_rendering()

    top_right_ticklabel_right_adv = max(
        ticklabel.get_window_extent().transformed(inv_fig_adv).x1
        for ticklabel in ax_adv_top_right.get_yticklabels()
        if ticklabel.get_visible()
    )
    ...
    ax_adv_top_right.set_position(
        (
            ...
            top_right_pos_adv.width
            - (top_right_ticklabel_right_adv - right_axes_edge_adv),
    ...
    top_right_pos_adv = ax_adv_top_right.get_position()
    top_row_gap_adv = (
        top_column_wspace * (top_left_pos_adv.width + top_right_pos_adv.width) / 2
    )
    ...
    fig_adv.draw_without_rendering()
    inv_fig_adv = fig_adv.transFigure.inverted()
    left_rate_bbox_adv = left_outer_rate_label_adv.get_window_extent().transformed(
        inv_fig_adv
    )
    target_ticklabels_adv = [
        ticklabel
        for ticklabel in ax_adv_middle_left.get_yticklabels()
        if ticklabel.get_visible()
    ]
    ...
    ax_adv_bottom_left.errorbar(
        N_STEPS,
        err_U_means_adv[0],
        student_CI_ppf_one_sided * err_U_stderrs_adv[0],
        color="C1",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_adv_bottom_left.errorbar(
        N_STEPS,
        err_U_means_adv[1],
        student_CI_ppf_one_sided * err_U_stderrs_adv[1],
        color="C1",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_adv_bottom_left.errorbar(
        N_STEPS,
        err_U_means_adv[2],
        student_CI_ppf_one_sided * err_U_stderrs_adv[2],
        color="C1",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    ax_adv_bottom_center.errorbar(
        N_STEPS,
        err_K_means_adv[0],
        student_CI_ppf_one_sided * err_K_stderrs_adv[0],
        color="C0",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_adv_bottom_center.errorbar(
        N_STEPS,
        err_K_means_adv[1],
        student_CI_ppf_one_sided * err_K_stderrs_adv[1],
        color="C0",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_adv_bottom_center.errorbar(
        N_STEPS,
        err_K_means_adv[2],
        student_CI_ppf_one_sided * err_K_stderrs_adv[2],
        color="C0",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    ax_adv_bottom_right.errorbar(
        N_STEPS,
        err_G_means_adv[0],
        student_CI_ppf_one_sided * err_G_stderrs_adv[0],
        color="C0",
        linewidth=0.5,
        linestyle="-",
        capsize=1,
    ...
    ax_adv_bottom_right.errorbar(
        N_STEPS,
        err_G_means_adv[1],
        student_CI_ppf_one_sided * err_G_stderrs_adv[1],
        color="C0",
        linewidth=0.5,
        linestyle="--",
        capsize=1,
    ...
    ax_adv_bottom_right.errorbar(
        N_STEPS,
        err_G_means_adv[2],
        student_CI_ppf_one_sided * err_G_stderrs_adv[2],
        color="C0",
        linewidth=0.5,
        linestyle=":",
        capsize=1,
    ...
    def _ylabel_x_adv(ax):
        bbox = ax.yaxis.label.get_window_extent().transformed(inv_fig_adv)
    ...
    panel_a_y_adv = _bbox_center_adv(
        top_outer_rate_label_adv.get_window_extent().transformed(inv_fig_adv)
    )[1]
    ...
    fig_adv.text(panel_a_x_adv, panel_a_y_adv, "(a)", fontdict=panel_label_style)
    fig_adv.text(panel_b_x_adv, panel_b_y_adv, "(b)", fontdict=panel_label_style)
    fig_adv.text(panel_c_x_adv, panel_c_y_adv, "(c)", fontdict=panel_label_style)
    fig_adv.text(panel_d_x_adv, panel_d_y_adv, "(d)", fontdict=panel_label_style)
    fig_adv.text(panel_e_x_adv, panel_e_y_adv, "(e)", fontdict=panel_label_style)
    fig_adv.text(panel_f_x_adv, panel_f_y_adv, "(f)", fontdict=panel_label_style)
    fig_adv.text(panel_g_x_adv, panel_g_y_adv, "(g)", fontdict=panel_label_style)
    fig_adv.text(panel_h_x_adv, panel_h_y_adv, "(h)", fontdict=panel_label_style)

    fig_adv.savefig(RES_DIR / ADVERSARIAL_EXAMPLE_FN, bbox_inches="tight")
