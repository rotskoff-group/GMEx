...
from gmex.utils import load_times_macrostates, get_results_dir, get_data_dir, get_bootstrap_curve_CI
...
def _get_even_odd_mean_dwells(
    ...
    gme_dwell_means = (
        gme_dwell_probs * torch.arange(max_timestep, device=device)
    ).sum(dim=1) * dt
    msm_dwell_means = (
        msm_dwell_probs * torch.arange(max_timestep, device=device)
    ).sum(dim=1) * dt
    return (
...
if __name__ == "__main__":
    ...
    dt_ATP, _, _, states_ATP, _ = load_times_macrostates(
        'BMF1/ATP_3mM', device='cpu'
    )
    ...
    dwells_ATPgS_mean = _get_even_odd_mean_dwells(Gs_ATPgS, dt_ATPgS, N_LAGS_TRUNC_ATPgS)
    ...
    bootstrap_dwell_buffer_ATPgS = np.empty((len(bootstrap_fns_ATPgS), 4), dtype=np.float64)
    for i in range(len(bootstrap_fns_ATPgS)):
        Gs_bootstrap_ATPgS = torch.load(bootstrap_fns_ATPgS[i], weights_only=True)
        bootstrap_dwell_buffer_ATPgS[i] = np.array(_get_even_odd_mean_dwells(
            Gs_bootstrap_ATPgS, dt_ATPgS, N_LAGS_TRUNC_ATPgS
        ))
    dwells_ATPgS_lower, dwells_ATPgS_upper = get_bootstrap_curve_CI(bootstrap_dwell_buffer_ATPgS, CI_PCT)
    ...
    for i in range(len(bootstrap_fns_ATP)):
        ...
        bootstrap_dwell_buffer_ATP[i] = np.array(_get_even_odd_mean_dwells(
            Gs_bootstrap_ATP, dt_ATP, N_LAGS_TRUNC_ATP
        ))
    dwells_ATP_lower, dwells_ATP_upper = get_bootstrap_curve_CI(bootstrap_dwell_buffer_ATP, CI_PCT)
    ...
    outer = fig.add_gridspec(
        nrows=2, ncols=1, height_ratios=[1, 2.2]
    )
    ...
    axes = [
        ax_r1_0, ax_r1_1, ax_r1_2, ax_r1_3, ax_r1_4,
        ax_r2_0, ax_r2_1, ax_r2_2, ax_r2_3,
    ]
    for a in axes:
    ...
    ATPgS_plot_offset = (
        df_ATPgS['Angle'][ATPgS_start_frame]
        - df_ATPgS['Accum'][ATPgS_start_frame]
    )
    ax_r1_1.plot(
        dt_ATPgS * np.arange(ATPgS_n_frames_plotted),
        df_ATPgS['Accum'][ATPgS_start_frame - 1:ATPgS_end_frame]
        + ATPgS_plot_offset,
        'k'
    )
    ...
    ax_r1_1.text(
        0.9, 0.05, r'ATP$\gamma$S',
        transform=ax_r1_1.transAxes,
        ha='right', va='bottom',
        fontsize=8
    )

    bin_edges = np.arange(145) * 2.5
    ax_r1_2.hist(
    ...
    ATP_plot_offset = (
        df_ATP['Angle'][ATP_start_frame]
        - df_ATP['Accum'][ATP_start_frame]
    )
    ax_r1_3.plot(
    ...
    ### panel (d) ###
    max_feasible_dwell_ATPgS = (
        np.ptp(df_ATPgS['Time']) / np.ptp(df_ATPgS['Revo']) / 3.0
    )
    ax_r2_2.errorbar(
    ...
    panel_b_x = fig.transFigure.inverted().transform(
        (angle_ylabel_bbox.x0, angle_ylabel_bbox.y1)
    )[0] - 0.005
    dwell_legend_text_bboxes = [
        text.get_window_extent(renderer) for text in dwell_legend.get_texts()
    ]
    dwell_legend_first_row_y = fig.transFigure.inverted().transform((
        0,
        max(
            (bbox.y0 + bbox.y1) / 2
            for bbox in dwell_legend_text_bboxes
        )
    ))[1]
    fig.text(
        panel_b_x, r1_legend_y, '(b)',
        ha='left',
        va='center',
        fontsize=panel_label_fontsize
    )
    fig.text(
        panel_a_x, dwell_legend_first_row_y, '(c)',
        ha='left',
        va='center',
        fontsize=panel_label_fontsize
    )
    fig.text(
        panel_b_x, dwell_legend_first_row_y, '(d)',
        ha='left',
        va='center',
        fontsize=panel_label_fontsize
    )
    ...
    print(f'BMF1 figure saved to {str(RES_DIR / FIG_FN)}')
