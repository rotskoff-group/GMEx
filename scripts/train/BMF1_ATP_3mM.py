#!/usr/bin/env python3
# GMEx/scripts/train/BMF1_ATP_3mM.py

"""
Train TCL-GME-DT models on trajectories of bovine mitochondrial F1 ATPase in 3mM ATP from
...
This script takes about 12 hrs to run.
"""
...
RESAMPLING = 1 # we want a 1 / 45 ms timestep and data are sampled at 1 / 45 ms
N_PER_SEG = 684 # we want 57 records of 15.2 ms each from a 39011-sample trajectory
N_LAGS_TOTAL = 16 # number of lags for which to compute count matrices
N_LAGS_TRUNC = 5 # TCL-GME-DT propagator plateaus at lag of 1 / 9 ms
...
DATA_DIR = get_data_dir() / 'BMF1/ATP_3mM'
RES_DIR = get_results_dir() / 'BMF1/ATP_3mM' # directory for saving results
MAIN_RES_GS_FN = 'BMF1-ATP-3mM-M6-L16-TCL-Gs.pt' # filename to save mean TCL-GME-DT propagators
MAIN_RES_MD_FN = 'BMF1-ATP-3mM-M6-L16-TCL-Gs-metadata.json' # filename to save metadata for preceding
BOOTSTRAP_DIR = RES_DIR / 'bootstraps' # directory for saving bootstrap results
BOOTSTRAP_GS_FN = 'BMF1-ATP-3mM-M6-L5-TCL-Gs-BS*.pt' # filename to save bootstrap TCL-GME-DT propagators
BOOTSTRAP_MD_FN = 'BMF1-ATP-3mM-M6-L5-TCL-Gs-BS*-metadata.json' # filename to save metadata for preceding
...
if __name__ == "__main__":
    ...
    print(f'BMF1 (3mM ATP) models saved to {str(RES_DIR)}')
