...
N_PER_SEG = 4860  # we want 314 records of 972 ns each from a 1526041-sample trajectory
...
MAIN_RES_MD_FN = (
    "HP35-M3-L64-TCL-Gs-metadata.json"  # filename to save metadata for preceding
)
...
BOOTSTRAP_MD_FN = (
    "HP35-M3-L32-TCL-Gs-BS*-metadata.json"  # filename to save metadata for preceding
)
...
if __name__ == "__main__":
    ...
    traj = traj[1:]  # drop initial sample: 1,526,041 -> 1,526,040 samples
    data = [
    ...
    for i in bootstrap_iter:
        ...
        Gs, info = get_Gs_mle(
            ...
            reversible=REVERSIBLE,
            verbose=False,
            postfix_callback=lambda postfix, i=i: bootstrap_iter.set_postfix(
                {"bootstrap": i, **postfix}
            ),
        )
    ...
    print(f"HP35 models saved to {RES_DIR!s}")
