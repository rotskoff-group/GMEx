...
from gmex.utils import load_times_macrostates, get_data_dir, get_results_dir, get_split
...
MAX_ITERS_PROJ = 100000 # maximum outer projection iterations per mirror-descent iteration
...
if __name__ == "__main__":
    ...
    for toyname in TOYNAMES:
        ...
        for n in N_STEPS: # iterate over sample sizes
            ...
            with catch_warnings():
                ...
                sample_iter = tqdm(range(len(trajs)), desc=f'Samples {n:.0e}', leave=False)
                for i in sample_iter: # iterate over trajectories
                    dataset = get_split(
                        dt, data=trajs[i][1:n + 1], train_frac=1.0, context_length=1, verbose=False
                    )
                    ...
                    Gs, Gs_info = get_Gs_mle(
                        ...
                        reversible=REVERSIBLE,
                        precomputed=None, # we don't warm-start here since the point is to show convergence
                        verbose=False,
                        postfix_callback=lambda postfix: sample_iter.set_postfix({'rep': i, **postfix})
                    )
    ...
    print(f'Toy-model results saved to {str(RES_DIR)}')
